...
show_menu
	...
	0) clear;
	    ...
	    hive -e "use finalproject; select worksite,count(case_status) as total,year from h1b_final where job_title='DATA ENGINEER' and year=$var group by worksite,year order by total desc limit 1;
"
        show_menu;
        ;;
	...
	4) clear;
	    ...
	    hive -e "use finalproject;  select worksite,count(case_status) as t,year from h1b_final where year =$var and case_status='CERTIFIED' group by worksite,year order by t desc limit 5;
"
       show_menu;
        ;;  
	...
	5) clear;
        option_picked "3) Which industry has the most number of Data Scientist positions?";
             hadoop jar nyse.jar Qs3 /niit2/h1b_kaggle.csv /niit2/out7
       show_menu;
        ;;
	...
	10) clear;
	    ...
	    hive -e "use finalproject; select year, COUNT(year) as number_of_applications from h1b_final group by year order by year;"
        show_menu;
        ;;
	...
	11) clear;
        ...
        echo "The average prevailing wage for each job for each year:";
	hive -e "use finalproject; select job_title,full_time_position,year,avg(prevailing_wage) as average from h1b_final where full_time_position ='Y' and year='2011' group by job_title,full_time_position,year order by average desc;"
        show_menu;
        ;;
	...
	pig -x local /home/hduser/qs9.pig	
        show_menu;
        ;;
	...
	option_picked "10) Which are the top 10 job positions which have the  success rate more than 70% in petitions and total petitions filed more than 1000?"
		pig -x local /home/hduser/qs10.pig
        show_menu;
        ;;
